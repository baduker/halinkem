...
██║  ██║██║  ██║███████╗██║██║ ╚████║██║  ██╗███████╗██║ ╚═╝ ██║
╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝'em all!                                                                                      
a comic scraper for tajo by baduker | v. 0.2 | github.com/baduker/halinkem
"""
...
def get_all_comic_src_urls(main_urls):
  ...
  for page in range(len(main_urls)):
    print_progress(counter, len(main_urls), prefix = 'Finding Halinki:', suffix = 'Compelted!', bar_length = 18)
...
def main():
  ...
  counter = 0
  for i in flat_list:
    print_progress(counter, len(flat_list), prefix = 'Downloading Halinki:', suffix = 'Completed!', bar_length = 18)
    download_engine(i)
    counter += 1

  end = time.time()
  print("\nFinished! {} Halinki downloaded in {:.2f} seconds.".format(len(flat_list), end - start))
